...
class Moderator:
    def __init__(self):
        ...
        self.public_round_log: List[str] = []  # 当前轮次的公开日志
    ...
    def log(self, event: str, public: bool = True):
        ...
        if public:
            ...
            self.public_round_log.append(log_entry)
        print(log_entry)
    
    def flush_log(self):
        """推送当前轮次的公开日志到飞书"""
        if self.public_round_log:
            message = "\n".join(self.public_round_log)
            self.feishu.send_message(message)
            self.public_round_log = []
    ...
    def night_phase(self):
        ...
        # 夜晚结束，推送日志（只推送公开信息，但这里全是私有信息，所以不推送）
        self.public_round_log = []
    ...
    def play_game(self):
        """运行游戏"""
        self.feishu.send_message("🐺 狼人杀游戏开始！\n9人局：3狼人、3村民、1预言家、1女巫、1猎人")
        ...
        while True:
            ...
            if game_over:
                ...
                self.flush_log()
                self.feishu.send_message(f"🎉 游戏结束，{winner}阵营胜利！")
                break
